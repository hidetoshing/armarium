...
Armariumは、ラテン語で「書物を収める戸棚」を意味する言葉です。電子書籍を整理し、身近な場所に保管しておくための小さな書棚として、この名前を付けました。
...
`.zip` は画像ファイルを収録した書庫として扱い、OPDSおよびダウンロードではCBZのMIMEタイプ `application/vnd.comicbook+zip` を使用します。ダウンロード時のファイル名も `.cbz` になりますが、ライブラリ内の元ファイルは変更しません。書庫内容の検証や、テキスト書庫など別形式からの変換は行いません。

ダウンロードレスポンスでは、幅広いOPDSクライアントとの互換性のため、ASCIIのフォールバックファイル名とUTF-8の元ファイル名を併記します。

OPDSの書籍タイトルには配信形式の拡張子を表示します。ZIPのタイトルには、CBZとして配信されることを示す `.cbz` を表示します。

OPDSの書籍情報には配信形式、正確なファイルサイズ、配信ファイル名を専用の要素・属性で出力します。CBZとZIPでは、書庫内の対応画像ファイル数をページ数相当として出力します。概要用の `content` と `summary` にはファイル情報を入れません。
