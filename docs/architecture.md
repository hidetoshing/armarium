...
メタデータキャッシュには抽出したタイトルを保存し、OPDSへ出力するときだけ配信形式の拡張子を付加します。ZIPには `.zip` ではなく `.cbz` を付加します。同じ拡張子が既にある場合は、大文字・小文字を区別せず重複を避けます。

CBZとZIPの画像数には `.jpg`、`.jpeg`、`.png`、`.gif`、`.webp`、`.avif` を数え、ページ数相当としてOPDSの `dcterms:extent` に出力します。見開き画像などの内容は解析せず、書庫内の対応画像ファイル1件を1ページとして数えます。画像数はキャッシュ内にも保持します。

OPDSの書籍エントリーでは、配信形式を `dc:format`、正確なバイト数を取得リンクの `length`、配信ファイル名を取得リンクの `title` に出力します。`content` と `summary` はファイル情報の表示には流用せず、概要や要約を取得できるようになるまで省略します。

ZIPはすべて画像書庫とみなし、CBZと同じMIMEタイプで配信します。ダウンロード時は拡張子を `.cbz` に置き換えたファイル名を `Content-Disposition` で提示しますが、ライブラリ内の元ファイルは変更しません。`Content-Disposition` には、従来形式向けのASCIIフォールバック `filename` と、元のファイル名をUTF-8で保持する `filename*` を併記します。Armariumは書庫形式の変換を責務とせず、書庫内容が画像だけで構成されているかも検証しません。

## 10. キャッシュ設計

キャッシュはプロセス内のマップとJSONファイルで構成されます。書籍エントリーのキーは電子書籍の絶対パス、ディレクトリエントリーのキーは閲覧したディレクトリの絶対パスです。

```json
{
  "version": 3,
  "directories": {
    "/books/books": 1783785600000000000
  },
  "entries": {
...
OPDSでディレクトリを開くたびに、そのディレクトリの更新日時を保存済みの値と比較します。更新されている場合は直下の通常ファイルだけを照合し、新規または変更された書籍のメタデータを抽出して、存在しなくなった直下書籍のエントリーを削除します。サブディレクトリは再帰的に処理せず、そのサブディレクトリが閲覧されたときに同じ処理を行います。

書籍キャッシュのヒット条件は、ファイルサイズと更新日時のUnixナノ秒値が両方一致することです。キャッシュ更新時は一時ファイルへモード `0600` で書き込んでからリネームします。キャッシュ操作はプロセス内のmutexで直列化します。旧形式のJSONキャッシュは読み込み可能で、ディレクトリを最初に閲覧した際に現行形式へ移行します。

キャッシュファイルの読み込み、解析、ディレクトリ作成、書き込みに失敗した場合は処理を継続します。このため、キャッシュは可用性に必須ではありませんが、現行実装ではキャッシュ障害をログへ出力しません。
